...
num_letters = 26
.data
;// Data only used for testing purposes. No procedure depends on global variables.
...
msg_2			BYTE "Tiol ol qf tbzktdtsn sgfu dtllqut ziqz voss fttr zg wt rtegrtr", 0

msg_11			BYTE "rugby ball ", 0
msg_22			BYTE "This is an extremely long message that will need to be decoded.", 0
number_mode2	DWORD -2
number_mode1	DWORD -1

;// Test case 2.

key_map_no_null2 BYTE "mnbvcxzasdfghjklpoiuytrewq"
msg_3			BYTE "Gk Bcmtcoi!!!", 0
msg_4			BYTE "'Hsiukow vkci jku oclcmu suicgx, nyu su kxucj oawhci' - Mmof Trmsj.", 0

msg_33			BYTE "Go Beavers!!!", 0
msg_44			BYTE "'History does not repeat itself, but it often rhymes' - Mark Twain.", 0
msg_55			BYTE "It is very fun to write messages and see how they start, get encrypted,",
						" and how the get decrypted again.",0
test1_16		WORD  -32769   ;// -32769 < x < 32 767
test2_16		WORD  -32768	;// -32769 < x < 32 767
number_mode0	DWORD 0

;// Testing for the key generation operation mode.
...
.code
main			PROC
	mov		edx, OFFSET	msg_55
	call	WriteString

	call	Crlf
	call	Crlf
	
	;// Testing for Key Generation.
	push	OFFSET key_map_no_null2
	push	OFFSET test_mode
	call	Compute



;// Testing for Decryption.
	push	OFFSET key_map_no_null2			;// [ebp + 16]
	push	OFFSET msg_55					;// [ebp + 12]
	push	OFFSET number_mode2				;// [ebp + 8]
...
;// Testing for Encryption.
	push	OFFSET key_map_no_null2			;// [ebp + 16]
	push	OFFSET msg_55					;// [ebp + 12]
	push	OFFSET number_mode1				;// [ebp + 8]
...
;// Testing for Decoy:
	push	test1_16
	push	test2_16
	push	OFFSET number_mode0
	call	Compute

	mov		eax, number_mode0
	call	WriteInt
